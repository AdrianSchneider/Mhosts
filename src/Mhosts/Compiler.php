...
class Compiler
{
     ...
     */
    public function compile($pharFile = 'mhosts.phar') 
    {
        if (file_exists($pharFile)) {
            unlink($pharFile);
        }
        
        $phar = new \Phar($pharFile, 0, 'mhosts.phar');
    ...
    {
        $content = file_get_contents(__DIR__.'/../../bin/mhosts');
        $content = preg_replace('{^#!/usr/bin/env php\s*}', '', $content);
        $phar->addFromString('bin/mhosts', $content);
    }
...
Phar::mapPhar('mhosts.phar');        
require 'phar://mhosts.phar/bin/mhosts';
